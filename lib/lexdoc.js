...
class Fulldoc {
  constructor(chevrotain) {
    this.chevrotain = chevrotain
  ...
  mode(name, def) {
    this.modes[name] = new TokenSet(this.chevrotain, def)
  ...
  createResult(order, tokens) {
    const instance = new this.chevrotain.Lexer(order)
    const result = {
      instance,
      tokens,
      lex: function(text) {
  ...
  buildSingleMode(def) {
    ...
    const ts = new TokenSet(this.chevrotain, def)
...
const Lexdoc = function(...args) {
  ...
  this.CATEGORY = this.__FD.chevrotain.Lexer.NA
  this.SKIPPED = this.__FD.chevrotain.Lexer.SKIPPED
}
